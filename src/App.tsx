...
const generateFullGridMap = async () => {
  ...
  const runNumericalTest = async () => {
    ...
    try {
      ...
      alert('Test completed successfully:' + JSON.stringify(testResults, null, 2));
    } catch (error) {
 ...
 // USED TO ALERT USERS NOT TO USE BRUTE FORCE WHEN NUMBER OF END NODES > 9
  useEffect(() => {
    if (endNodes.length > 10) {
      alert('Warning: Brute Force takes quite long to run when the number of end nodes exceeds 10. I highly recommend users to use the Nearest Neighbor algorithm instead.');
    }
  }, [endNodes.length]);
  return (
      ...
      >
        ...
        <fieldset style={{ marginBottom: '12px', border: '1px solid #ccc', padding: '8px' }}>
          ...
          <div style={{ fontSize: '11px', color: '#666', marginTop: '4px' }}>
            - Firstly, users select a portion of map at <a href="https://overpass-turbo.eu/s/26Oq" target="_blank" rel="noopener noreferrer">overpass-turbo</a> <br/>
  ...
  <div style={{ fontSize: '11px', color: '#666' }}>
    Generate a random maze or a random full grid for pathfinding algorithms on unweighted graph.
        ...
        <fieldset style={{ marginBottom: '12px', border: '1px solid #ccc', padding: '8px' }}>
          ...
          <div style={{ fontSize: '11px', color: '#666', marginTop: '4px' }}>
            Run numerical performance test for algorithms on a predownloaded <a href="https://overpass-turbo.eu/s/26Oc" target="_blank" rel="noopener noreferrer">Dong Xoai city map</a> 15 times with different random start and end nodes each time. Open console to see results.
            ...
            <>
              ...
              <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #ddd' }}>
                ...
                <div style={{ fontSize: '10px', color: '#666', marginTop: '2px' }}>
                  Format: letters, numbers, underscores, hyphens only. Refer to <a href="https://wiki.openstreetmap.org/wiki/Key:highway" target="_blank" rel="noopener noreferrer">OSM Map Highway Type</a> for more valid and conventional types. 
                </div>
              </div>
            </>
          )}
